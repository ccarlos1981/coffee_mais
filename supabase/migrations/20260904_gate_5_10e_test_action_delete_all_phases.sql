-- Migration: 20260904_gate_5_10e_test_action_delete_all_phases.sql
-- Description: Gate 5.10E - Atualização da RPC soberana excluir_acao_investimento_teste_v1 para permitir exclusão administrativa de ações de teste (is_test = TRUE) em todas as fases operacionais (Fases 1 a 6) exclusivamente para perfis Trade e Admin, preservando integralmente o Financial Guard e a proteção contra ações reais.
...
DECLARE
    ...
    v_fase INTEGER;
BEGIN
    -- [A] Identificação Canônica do Usuário e Prevenção de Spoofing
    v_auth_uid := auth.uid();
    ...
    -- [D] Condição Fundamental: Ação DEVE estar explicitamente classificada no banco como is_test = TRUE
    ...
    -- [E] Validação de Fases Permitidas (Gate 5.10E: Fases 1 a 6 são permitidas para Trade/Admin quando is_test = TRUE)
    v_fase := COALESCE(v_acao.fase_atual, 1);
    IF v_fase < 1 OR v_fase > 6 THEN
        RAISE EXCEPTION 'Operação Bloqueada: Fase inválida (%).', v_fase;
    END IF;

    -- [F] Bloqueio Financeiro Absoluto (Financial Guard: Nenhuma dependência financeira permitida)
