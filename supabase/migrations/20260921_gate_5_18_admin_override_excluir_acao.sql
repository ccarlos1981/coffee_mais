-- Migration: 20260921_gate_5_18_admin_override_excluir_acao.sql
-- Description: Gate 5.18 - Evolução da RPC soberana excluir_acao_investimento_admin_v2 com suporte a override administrativo (p_admin_override), restrito a Admin e Admin Master, permitindo exclusão cirúrgica de ações em negociações multi-ações sem corromper vínculos ou inventar rateios.
-- Baseline: BASELINE_INVESTIMENTOS_20260909_LOCKED & GATE_5_16_CONGELADO

-- ==============================================================================
-- 1. DROP DA ASSINATURA ANTERIOR DE 4 PARÂMETROS PARA EVITAR AMBIGUIDADE
-- ==============================================================================
DROP FUNCTION IF EXISTS public.excluir_acao_investimento_admin_v2(UUID, TEXT, UUID, BOOLEAN);

-- ==============================================================================
-- 2. CRIAÇÃO DA RPC SOBERANA COM SUPORTE A p_admin_override
-- ==============================================================================
...
BEGIN
    ...
    -- [B] Validação do Perfil e Role (RBAC Estrito: Trade, Admin, Admin Master)
    ...
    IF TRIM(LOWER(v_user_profile.role)) NOT IN ('trade', 'admin', 'admin master') THEN
        RAISE EXCEPTION 'Acesso Negado: Apenas os perfis Trade, Admin e Admin Master possuem autorização para exclusão administrativa de ações (role atual: "%").', v_user_profile.role;
    END IF;

    -- [B.1] Validação Específica do Override Administrativo
    IF p_admin_override IS TRUE THEN
        IF TRIM(LOWER(v_user_profile.role)) NOT IN ('admin', 'admin master') THEN
            RAISE EXCEPTION 'Acesso Negado: Apenas perfil Admin ou Admin Master possui autorização para override administrativo de exclusão (role atual: "%").', v_user_profile.role;
        END IF;
    END IF;
    ...
    -- =========================================================================
    -- CASO 0: ADMIN OVERRIDE (Exceção Administrativa Controlada e Auditada)
    -- =========================================================================
    IF p_admin_override IS TRUE THEN
        -- 1. Remoção de Vínculos Auxiliares exclusivos da ação
        ...
        -- 2. Registro de Auditoria Forense Obrigatória (ADMIN_OVERRIDE_MULTI_ACAO)
        INSERT INTO public.cm_audit_logs (
            ...
            jsonb_build_object(
                ...
                'classificacao_sobrescrita', v_classificacao,
                'valor_investimento', v_acao.valor_investimento,
                'outras_acoes_ativas_count', v_outras_acoes_ativas_count,
                'motivo_exclusao', COALESCE(p_motivo, 'Exclusão administrativa via override autorizado de governança multi-ações'),
        ...
        -- 3. Exclusão Física da Ação Alvo (Apenas ela é removida; as demais ações permanecem intactas)
        DELETE FROM public.cm_acoes_investimento WHERE id = p_acao_id;

        -- 4. Reconciliação Oficial da Campanha Pai (sem inventar rateio ou alterar parcelas compartilhadas)
        IF v_acao.campanha_id IS NOT NULL THEN
            SELECT 
            ...
            IF v_novo_projetado = 0.00 THEN
                ...
                -- Se não resta nenhuma ação ativa na campanha e não houve pagamento, cancela parcelas pendentes
                IF v_campanha_pagamentos_count = 0 AND v_parcelas_pagas_count = 0 THEN
                    UPDATE public.cm_investimento_parcelas
                    SET
                        status_parcela = 'CANCELADA_EXCLUSAO_ACAO',
                        saldo_remanescente = 0.00,
                        observacoes = COALESCE(observacoes || ' | ', '') || 'Cancelada por cancelamento total da campanha master via override',
        ...
        RETURN jsonb_build_object(
            'success', true,
            'operation', 'ADMIN_OVERRIDE_DELETE',
            'code', 'OVERRIDE_SUCCESS',
            'acao_id', p_acao_id,
            'campanha_id', v_acao.campanha_id,
            'deleted_rows', 1,
            'is_test', COALESCE(v_acao.is_test, false),
            'valor_investimento_total', v_novo_projetado,
            'saldo_financeiro_devedor', v_novo_devedor,
            'outras_acoes_restantes', v_outras_acoes_ativas_count,
            'message', 'Ação excluída com sucesso via override administrativo autorizado.'
        );

    -- =========================================================================
    -- CASO 1: PARTIAL_REALIZED (Soft-Cancel com Blindagem do Gate 5.15B)
    -- =========================================================================
    ELSIF v_classificacao = 'PARTIAL_REALIZED' THEN
...
-- ==============================================================================
-- 3. AJUSTE DE PRIVILÉGIOS E GRANTS DA RPC v2 EVOLUÍDA
-- ==============================================================================

REVOKE ALL ON FUNCTION public.excluir_acao_investimento_admin_v2(UUID, TEXT, UUID, BOOLEAN, BOOLEAN) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.excluir_acao_investimento_admin_v2(UUID, TEXT, UUID, BOOLEAN, BOOLEAN) FROM anon;
GRANT EXECUTE ON FUNCTION public.excluir_acao_investimento_admin_v2(UUID, TEXT, UUID, BOOLEAN, BOOLEAN) TO authenticated, service_role;
