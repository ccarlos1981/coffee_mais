-- ==============================================================================
-- MIGRATION: 20260913_gate_5_17_fase3_fechamento_completo_atomico.sql
-- RDM GATE 5.17 - FASE 3.1: RPC TRANSACIONAL DE FECHAMENTO COMPLETO
-- Operação Canônica: Fechamento Simultâneo Atômico (Apuração + Plano Financeiro)
-- 
-- Transação Única PostgreSQL:
-- 1. Validação de Autenticação / RBAC / Anti-Spoof
-- 2. Advisory Lock de Idempotência (p_idempotency_key)
-- 3. Lock da Ação (SELECT FOR UPDATE)
-- 4. Lock da Campanha (Advisory Lock + SELECT FOR UPDATE)
-- 5. Verificação de Idempotência (cm_audit_logs)
-- 6. Validação da Apuração (acordo obrigatório, fase 3/4)
-- 7. Validação dos Boletos (existência de cada boleto)
-- 8. Gravação da Apuração em cm_acoes_investimento
-- 9. Vínculos de Boletos em cm_acoes_boletos_vinculo (Ownership Exclusivo Comercial)
-- 10. Verificação de Multi-Action (Caminho A: Parcial vs Caminho B: Completo)
-- 11. Cálculo do Total Financeiro Canônico (SSOT: SUM(valor_investimento) ações ativas)
-- 12. Validação da Grade de Parcelas (A_VISTA = 1 parcela, PARCELADO = N parcelas, tolerância R$ 0,01)
-- 13. Criação Atômica das Parcelas em cm_investimento_parcelas
-- 14. Atualização da Campanha (tipo_plano, saldo_financeiro_devedor, valor_total_projetado)
-- 15. Auditoria Forense em cm_audit_logs
-- 16. Retorno Estruturado
--
-- Reversibilidade:
-- DROP FUNCTION IF EXISTS public.concluir_fechamento_investimento_completo_v1(UUID, TEXT, INTEGER, NUMERIC, TEXT, TEXT, BOOLEAN, TEXT, JSONB, JSONB, TEXT, UUID, TEXT);
-- ==============================================================================
...
BEGIN
    ...
    IF v_is_caminho_b THEN
        ...
        -- Limpar parcelas pendentes anteriores da campanha se houver
...
'RDM Gate 5.17 Fase 3.1: Unidade transacional atômica para fechamento de investimento. Executa apuração comercial, vínculos de boleto (ownership exclusivo), validação de prontidão multi-action, cálculo canônico SSOT (SUM(valor_investimento)), validação e geração de parcelas, atualização de campanha e auditoria forense sob uma única transação PostgreSQL.';
