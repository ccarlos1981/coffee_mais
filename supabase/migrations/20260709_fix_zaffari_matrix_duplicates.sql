-- Update v_redes_matrizes_detalhes to group by (codigo_matriz, matriz, responsavel) to allow multiple networks with same code but different names/managers (e.g., Zaffari under different managers/regions)
DROP VIEW IF EXISTS public.v_redes_matrizes_detalhes CASCADE;

CREATE VIEW public.v_redes_matrizes_detalhes AS
 SELECT DISTINCT ON (codigo_matriz, matriz, responsavel) codigo_matriz AS codigo,
    matriz AS nome,
    tipo_parceiro AS canal,
    codigo::text AS min_cod_parceiro,
    responsavel AS gerente,
    uf,
    regional,
    created_at,
  ...
  ORDER BY codigo_matriz, matriz, responsavel, created_at DESC;

-- Update v_acoes_investimento_com_gerente to map gerente_responsavel by both code and network name
