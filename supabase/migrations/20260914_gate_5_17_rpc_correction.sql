-- ==============================================================================
-- MIGRATION: 20260914_gate_5_17_rpc_correction.sql
-- RDM GATE 5.17 - FASE 4E.1: CORREÇÃO DA RPC DE FECHAMENTO COMPLETO
-- Substituição da definição efetiva no banco pela definição canônica homologada
--
-- Motivo: A versão registrada anteriormente em produção (20260913193422) continha
-- um draft desatualizado com referências a colunas inexistentes:
-- 1. apuracao_data (correto: apuracao_preenchida_em)
-- 2. primeiro_boleto_id (correto: apuracao_boleto_id)
-- 3. cm_acoes_boletos_vinculo.vinculado_por (coluna inexistente no schema)
-- 4. status_operacional em cm_acoes_investimento (correto: cancel_reason IS NULL)
-- 5. Bloqueio indevido no Caminho A (p_plano_financeiro IS NULL agora preservado)
--
-- Reversibilidade:
...
BEGIN
    ...
    -- Inserir novos vínculos de boletos (apenas colunas existentes no schema canônico)
...
'RDM Gate 5.17 Fase 4E.1: Correção definitiva da RPC de fechamento atômico de investimento. Alinha a definição do banco com o schema canônico (apuracao_preenchida_em, apuracao_boleto_id, cm_acoes_boletos_vinculo sem vinculado_por, cancel_reason IS NULL e preservação do Caminho A com p_plano_financeiro IS NULL).';
