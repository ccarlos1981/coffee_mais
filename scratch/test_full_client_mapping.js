...
async function testFullClientMapping() {
  ...
  managers.forEach(mName => {
    ...
    const sortedClients = Array.from(set).sort((a, b) => a.localeCompare(b, 'pt-BR'));
    sortedClients.push('OUTROS');

    console.log(`\nGerente: ${mName} -> Total Clientes na Carteira Completa: ${sortedClients.length}`);
    console.log('Primeiros 10 clientes:', sortedClients.slice(0, 10));
    console.log('Últimos 5 clientes (deve ter OUTROS no final):', sortedClients.slice(-5));
  });
}

testFullClientMapping();
