...
async function testRedesOnly() {
  ...
  // Buscar somente REDE válida de mv_vendas_cliente_mensal
  ...
  // Buscar todas as vendas sem REDE para agrupamento em OUTROS
  const sqlNonRedeHistory = `
    SELECT 
      mes,
      manager,
      SUM(fat) as fat
    FROM mv_vendas_cliente_mensal
    WHERE mes IN ('${curMonthKey}', '${prevMonthKey}', '${prevYearKey}', '${closedMonth2}', '${closedMonth3}')
      AND (rede IS NULL OR TRIM(rede) = '')
    GROUP BY mes, manager
  `;

  // Buscar REDES oficiais em base_atendimento
  ...
  const [resCliHist, resNonRede, resBaseCli] = await Promise.all([
    supabase.rpc('execute_readonly_query', { query_text: sqlClientHistory }),
    supabase.rpc('execute_readonly_query', { query_text: sqlNonRedeHistory }),
    supabase.rpc('execute_readonly_query', { query_text: sqlBaseClients })
  ]);

  const cliHist = resCliHist.data || [];
  const nonRedeHist = resNonRede.data || [];
  ...
  managers.forEach(mName => {
    ...
    const sortedRedes = Array.from(set).sort((a, b) => a.localeCompare(b, 'pt-BR'));
    sortedRedes.push('OUTROS');

    console.log(`\n==================================================`);
    console.log(`GERENTE: ${mName} -> TOTAL DE REDES COMERCIAIS: ${sortedRedes.length - 1} (+ OUTROS)`);
    console.log(`==================================================`);
    console.log(sortedRedes);
  });
}
