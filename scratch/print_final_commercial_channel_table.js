...
async function printFinalReport() {
  const { data: mvData } = await supabase.rpc('execute_readonly_query', {
    query_text: `
      SELECT 
        channel,
        SUM(fat) as fat_real,
  ...
  const { data: rawData } = await supabase.rpc('execute_readonly_query', {
    query_text: `
      WITH base AS (
        SELECT 
          f.cod_top,
  ...
  const grandTotalReal = mvData.reduce((acc, r) => acc + Number(r.fat_real || 0), 0);
  const grandTotalBruto = rawData.reduce((acc, r) => acc + Number(r.bruto || 0), 0);
  const grandTotalDev = rawData.reduce((acc, r) => acc + Number(r.devolucoes || 0), 0);
  const grandTotalBon = rawData.reduce((acc, r) => acc + Number(r.bonificacoes || 0), 0);
  const grandTotalFut = mvData.reduce((acc, r) => acc + Number(r.venda_futura || 0), 0);
  const grandTotalQtd = rawData.reduce((acc, r) => acc + Number(r.volume_und || 0), 0);
  const grandTotalKg = rawData.reduce((acc, r) => acc + Number(r.volume_kg || 0), 0);

  console.log('--- CONSOLIDAÇÃO FINAL ---');
  console.log(`Grand Total Net Real: R$ ${grandTotalReal.toFixed(2)}`);

  const report = mvData.map(mv => {
    const raw = rawData.find(r => r.channel === mv.channel) || {};
    ...
    const und = Number(raw.volume_und || 0);
  ...
  console.log('TOTAL GERAL CONSOLIDADO:', {
...
printFinalReport().catch(console.error);
