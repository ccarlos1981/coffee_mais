...
export default function AcompAnualPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/dashboard/meta-cia?year=${YEAR}`);
  ...
  useEffect(() => {
    const checkAuthAndPermissions = async () => {
      try {
        ...
        console.error("Erro na verificação de permissões do módulo Meta Cia:", err);
        setHasAccess(false);
      } finally {
        setAuthLoading(false);
      }
  ...
  const handleUpdateMeta = async (month: number, channel: string, field: "target_forecast" | "target_internal", valueStr: string) => {
    ...
    try {
      if (existing && existing.id) {
        ...
        const payload: CeoTarget = {
          ...
          month,
          channel,
          target_forecast: field === "target_forecast" ? value : 0,
          target_internal: field === "target_internal" ? value : 0,
        };
  ...
  const buildJoinedData = (channelFilter: string): JoinedData[] => {
    return MONTHS.map((mName, idx) => {
      ...
      return {
        ...
        monthName: mName,
        real: r,
        forecast: t?.target_forecast || 0,
        internal: t?.target_internal || 0,
        dbId: t?.id,
      };
    });
  };
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-6 text-foreground">
        <div className="glass-card p-8 max-w-md w-full text-center relative overflow-hidden shadow-2xl border border-border">
          ...
          <p className="text-sm text-muted mb-6">
            Seu perfil ({userRole || "Não autenticado"}) não possui permissão para acessar o módulo Meta Cia (Painel do CEO).
          </p>
          <Link
            href="/"
  ...
  const renderTable = (dataLines: JoinedData[], type: "forecast" | "internal", channel: string) => {
    ...
    const saveField = isForecast ? "target_forecast" : "target_internal";
          ...
          <div className="text-right">
            ...
            <div className="flex items-end justify-end gap-3">
              <div className="flex flex-col items-end">
                <span className="text-sm font-semibold text-foreground">{formatCurrency(totalRealAno)}</span>
                <span className="text-xs text-muted">/ {formatCurrency(totalMetaAno)}</span>
              </div>
          ...
          <table className="w-full text-left text-sm border-collapse min-w-[600px]">
            <thead>
              <tr className="border-b border-border text-muted bg-muted/20 backdrop-blur-sm">
                ...
                <th className="font-semibold py-3 px-6 text-right w-48 uppercase text-xs tracking-wider">Meta (R$)</th>
                <th className="font-semibold py-3 px-6 text-right w-48 uppercase text-xs tracking-wider">Real (R$)</th>
            ...
            <tbody className="divide-y divide-border/30">
              ...
              <tr className="bg-muted/10 border-b-2 border-border/60">
                ...
                <td className="py-4 px-6 text-right font-bold text-foreground">
                  {formatCurrency(totalMetaYTD)}
                </td>
                <td className="py-4 px-6 text-right font-bold text-foreground tracking-tight">
                  {totalRealYTD > 0 ? formatCurrency(totalRealYTD) : <span className="text-dim">-</span>}
                </td>
                ...
                return (
                  <tr key={row.month} className="group hover:bg-muted/10 transition-colors">
                    ...
                    <td className="py-2.5 px-6 text-right font-medium text-foreground tracking-tight">
                      {row.real > 0 ? formatCurrency(row.real) : <span className="text-dim">-</span>}
                    </td>
      ...
      <header className="border-b border-border/60 bg-background/80 backdrop-blur-md sticky top-0 z-50">
        <div className="max-w-[1400px] mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-foreground tracking-tight">Meta Cia</h1>
              <p className="text-xs font-medium text-muted uppercase tracking-widest mt-0.5">Visão Executiva</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/meta-cia-unidades" className="text-sm font-bold text-foreground bg-blue-500/10 hover:bg-blue-500/20 px-4 py-2 rounded-full border border-blue-500/20 transition-all flex items-center gap-2">
                        ...
                        <div key={channel} className="glass-card flex flex-col overflow-hidden border border-border/50 rounded-2xl transition-all duration-300 bg-background/50">
                          ...
                          >
                            <div>
                              ...
                              <p className="text-xs text-muted text-left font-medium">{formatCurrency(totalReal)} gerado</p>
                            </div>
                                          ...
                                          <tr key={l.month} className="border-b border-border/30 last:border-0 hover:bg-muted/20 transition-colors">
                                            ...
                                            <td className="px-2 py-2.5">
                                              <input
                                                ...
                                                placeholder="0"
                                                onBlur={(e) => handleUpdateMeta(l.month, channel, "target_forecast", e.target.value)}
                                          ...
                                          <tr key={l.month} className="border-b border-border/30 last:border-0 hover:bg-muted/20 transition-colors">
                                            ...
                                            <td className="px-2 py-2.5">
                                              <input
                                                ...
                                                placeholder="0"
                                                onBlur={(e) => handleUpdateMeta(l.month, channel, "target_internal", e.target.value)}
                                                onKeyDown={(e) => e.key === "Enter" && e.currentTarget.blur()}
