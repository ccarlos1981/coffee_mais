...
interface ForecastOportunidadesCardProps {
  oportunidades: ForecastRiscoOportunidade[];
  onSelect: (item: ForecastRiscoOportunidade) => void;
  loading?: boolean;
}

export const ForecastOportunidadesCard: React.FC<ForecastOportunidadesCardProps> = ({
  oportunidades,
  onSelect,
  loading = false,
}) => {
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h3 className="text-sm font-bold text-foreground">Oportunidades de Superação</h3>
          <p className="text-[11px] text-muted-foreground">Alavancas comerciais para alavancar a projeção</p>
        </div>
      </div>
        ...
        {oportunidades.map((op) => (
          ...
          >
            <div className="flex justify-between text-xs">
              ...
              <span className="font-mono font-bold text-emerald-500">+{formatCur(op.impactoEstimado)}</span>
            </div>
