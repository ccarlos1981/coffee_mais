...
interface ForecastFilterBarProps {
  ...
  onFilterChange: (newFilters: ForecastFiltersState) => void;
  onReset: () => void;
  loading?: boolean;
}
  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        console.error("Erro ao carregar opções de filtro Forecast:", err);
      }
    }
    loadOptions();
  }, []);

  const handleChange = (key: keyof ForecastFiltersState, value: string) => {
    onFilterChange({
      ...filters,
      [key]: value,
    });
    ...
    <div className="bg-card border border-border rounded-2xl p-4 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">Filtros do Forecast Comercial</h3>
            <p className="text-[11px] text-muted-foreground">Projeções executivas filtradas por período e dimensões comerciais</p>
          </div>
        </div>
