...
export interface ExcluirAcaoModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (override?: boolean) => Promise<void> | void;
      ...
      >
        ...
        <div 
          className={`absolute top-0 left-0 right-0 h-1.5 ${
            diagnostico === "CLEAN" 
              ...
              ? "bg-orange-500"
              : (diagnostico === "MULTI_ACTION_FINANCIAL_AMBIGUOUS" && isAdmin)
              ? "bg-amber-500"
              : (diagnostico === "FULLY_REALIZED" && isAdmin)
              ? "bg-amber-500"
              : "bg-red-600"
          }`} 
        />
          ...
          <div className="flex items-start gap-3.5">
            <div className={`p-3 rounded-xl border ${
              ...
              {diagnostico === "CLEAN" ? (
                ...
                isAdmin ? <ShieldAlert className="w-6 h-6 text-amber-400" /> : <Layers className="w-6 h-6 text-red-400" />
              ) : (
                isAdmin ? <ShieldAlert className="w-6 h-6 text-amber-400" /> : <Lock className="w-6 h-6 text-red-400" />
              )}
            </div>
              ...
              <div className="flex items-center gap-2">
                <h3 className="text-lg font-bold text-foreground">
                  ...
                  {diagnostico === "FULLY_REALIZED" && (
                    isAdmin
                      ? `Excluir ${tipoEntidadeLabel} com Histórico Realizado?`
                      : "Exclusão Bloqueada — Financial Guard"
                  )}
                  {diagnostico === "MULTI_ACTION_FINANCIAL_AMBIGUOUS" && (
                    isAdmin
                      ? `Exclusão Administrativa — Negociação Multi-Ações`
        ...
        {diagnostico === "FULLY_REALIZED" && isAdmin && (
          <div className="space-y-3 text-xs bg-amber-950/30 p-4 rounded-xl border border-amber-500/40 text-amber-200/90">
            <p className="font-bold text-amber-400 flex items-center gap-2 text-sm">
              ...
              Ação com Histórico Realizado — Autorização de Administrador
            </p>
            <p className="leading-relaxed text-muted-foreground">
              Esta {tipoEntidadeLabel} possui liquidação financeira integral ou pagamentos baixados.
            </p>
            <div className="bg-amber-900/30 p-2.5 rounded-lg border border-amber-500/20 text-[11px] space-y-1">
              <p className="font-semibold text-amber-300">Atenção ao Impacto Contábil:</p>
              <p className="text-amber-100/70">
                Como Administrador, você possui permissão para executar a exclusão com override. A operação será registrada com auditoria completa no sistema.
              </p>
            </div>
            <p className="text-amber-300 font-semibold pt-1">
              Deseja prosseguir com a exclusão administrativa desta ação?
            </p>
          </div>
        )}
        ...
        {diagnostico === "MULTI_ACTION_FINANCIAL_AMBIGUOUS" && isAdmin && (
          <div className="space-y-3 text-xs bg-amber-950/30 p-4 rounded-xl border border-amber-500/40 text-amber-200/90">
            <p className="font-bold text-amber-400 flex items-center gap-2 text-sm">
              <ShieldAlert className="w-4 h-4 text-amber-400" />
              Ação em Negociação Multi-Ações — Autorização de Administrador
            </p>
            <p className="leading-relaxed text-muted-foreground">
              Esta ação pertence a uma negociação master com <strong>{action.outras_acoes_ativas_count || "múltiplas"} outra(s) ação(ões) ativa(s)</strong> e plano financeiro compartilhado.
            </p>
            <div className="bg-amber-900/30 p-3 rounded-lg border border-amber-500/30 text-[11px] space-y-2">
              <p className="font-semibold text-amber-300 flex items-center gap-1.5">
                <ShieldCheck className="w-3.5 h-3.5 text-amber-400" />
                Regras de Governança na Exclusão Administrativa:
              </p>
              <ul className="space-y-1 pl-1 text-amber-100/80">
                <li className="flex items-start gap-1.5">
                  <span className="text-amber-400 font-bold">•</span>
                  <span>Apenas esta ação individual será desvinculada e excluída fisicamente do sistema.</span>
                </li>
                <li className="flex items-start gap-1.5">
                  <span className="text-amber-400 font-bold">•</span>
                  <span>As demais {action.outras_acoes_ativas_count || "outras"} ações ativas permanecerão 100% íntegras e inalteradas.</span>
                </li>
                <li className="flex items-start gap-1.5">
                  <span className="text-amber-400 font-bold">•</span>
                  <span>O valor projetado e o saldo devedor da negociação master serão recalculados automaticamente.</span>
                </li>
                <li className="flex items-start gap-1.5">
                  <span className="text-amber-400 font-bold">•</span>
                  <span>Não haverá rateio arbitrário nem desbalanceamento nas demais ações.</span>
                </li>
                <li className="flex items-start gap-1.5">
                  <span className="text-amber-400 font-bold">•</span>
                  <span>Esta operação gera log de auditoria forense obrigatório (<code className="font-mono text-[10px] text-amber-300">ADMIN_OVERRIDE_MULTI_ACAO</code>).</span>
                </li>
              </ul>
            </div>
            <p className="text-amber-300 font-semibold pt-1">
              Como Administrador, confirma a exclusão definitiva desta ação individual?
            </p>
          </div>
        )}
        ...
        <div className="flex items-center justify-end gap-3 pt-3 border-t border-border/50">
          ...
          >
            {((diagnostico === "FULLY_REALIZED" || diagnostico === "MULTI_ACTION_FINANCIAL_AMBIGUOUS") && !isAdmin)
              ? "Entendido / Fechar"
              : "Cancelar"}
          </button>

          {/* Botão de confirmação para CLEAN, FUTURE_ONLY e PARTIAL_REALIZED */}
          ...
          {/* Botão de Override Administrativo (Admin / Admin Master) */}
          {isAdmin && (diagnostico === "MULTI_ACTION_FINANCIAL_AMBIGUOUS" || diagnostico === "FULLY_REALIZED") && (
            <button
              type="button"
              onClick={() => onConfirm(true)}
              disabled={isLoading}
              className="px-4 py-2 text-xs font-bold text-white bg-amber-600 hover:bg-amber-500 rounded-xl transition-all shadow-lg shadow-amber-950/30 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                  <span>Processando Override...</span>
                </>
              ) : (
                <>
                  <ShieldAlert className="w-4 h-4" />
                  <span>Excluir como Administrador</span>
                </>
              )}
            </button>
          )}
