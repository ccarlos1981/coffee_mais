...
export interface PresidencyFiltersState {
  startMonth: string;
  endMonth: string;
  manager: string;
  uf: string;
...
interface PresidencyHeaderProps {
  filters: PresidencyFiltersState;
  onFilterChange: (newFilters: PresidencyFiltersState) => void;
  onReset: () => void;
  loading?: boolean;
}

export const PresidencyHeader: React.FC<PresidencyHeaderProps> = ({
  filters,
  onFilterChange,
  onReset,
  loading = false,
  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        console.error("Erro ao carregar filtros da Presidência:", err);
      }
    }
    loadOptions();
  }, []);

  const handleChange = (key: keyof PresidencyFiltersState, value: string) => {
    onFilterChange({
      ...filters,
      [key]: value,
    });
    ...
    <div className="bg-card border border-border rounded-2xl p-4 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">Filtros da Presidência</h3>
            <p className="text-[11px] text-muted-foreground">Escopo de apuração executiva consolidada</p>
          </div>
        </div>
