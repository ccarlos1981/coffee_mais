...
import { QualityRecommendation } from "@/lib/governance/data-quality";

interface DataRecommendationsProps {
  recommendations: QualityRecommendation[];
}

export const DataRecommendations: React.FC<DataRecommendationsProps> = ({ recommendations }) => {
  return (
    <div className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4">
      <div className="flex items-center gap-2 border-b border-border pb-3">
        ...
        <div>
          <h3 className="text-sm font-bold text-foreground">Recomendações de Governança & Saneamento de Dados</h3>
          <p className="text-[11px] text-muted-foreground">
            Ações técnicas para otimização contínua da qualidade e completude da base
          </p>
        </div>
      </div>
