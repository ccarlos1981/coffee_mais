...
export interface AssistantFiltersState {
  startMonth: string;
  endMonth: string;
  manager: string;
  uf: string;
...
interface AssistantFilterBarProps {
  filters: AssistantFiltersState;
  onFilterChange: (newFilters: AssistantFiltersState) => void;
  onReset: () => void;
  loading?: boolean;
}

export const AssistantFilterBar: React.FC<AssistantFilterBarProps> = ({
  filters,
  onFilterChange,
  onReset,
  loading = false,
  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        console.error("Erro ao carregar filtros do Assistente:", err);
      }
    }
    loadOptions();
  }, []);

  const handleChange = (key: keyof AssistantFiltersState, value: string) => {
    onFilterChange({
      ...filters,
      [key]: value,
    });
    ...
    <div className="bg-card border border-border rounded-2xl p-4 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">Filtros do Assistente Comercial</h3>
            <p className="text-[11px] text-muted-foreground">Defina o escopo dos dados consultados pela IA Executiva</p>
          </div>
        </div>
