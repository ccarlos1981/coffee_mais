...
interface DreFilterBarProps {
  filters: DreFiltersState;
  onFilterChange: (newFilters: DreFiltersState) => void;
  onReset: () => void;
  loading?: boolean;
}

export const DreFilterBar: React.FC<DreFilterBarProps> = ({
  filters,
  onFilterChange,
  onReset,
  loading = false,
  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        console.error("Erro ao carregar opções de filtro DRE:", err);
      }
    }
    loadOptions();
  }, []);

  const handleChange = (key: keyof DreFiltersState, value: string) => {
    onFilterChange({
      ...filters,
      [key]: value,
    });
    ...
    <div className="bg-card border border-border rounded-2xl p-4 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">Filtros da DRE Comercial</h3>
            <p className="text-[11px] text-muted-foreground">Filtre o demonstrativo por período, gerente, regional, canal ou rede</p>
          </div>
        </div>
