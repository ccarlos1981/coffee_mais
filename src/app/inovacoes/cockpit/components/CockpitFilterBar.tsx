...
export interface CockpitFiltersState {
  startMonth: string;
  endMonth: string;
  manager: string;
  uf: string;
...
interface CockpitFilterBarProps {
  filters: CockpitFiltersState;
  onFilterChange: (newFilters: CockpitFiltersState) => void;
  onReset: () => void;
  loading?: boolean;
}

export const CockpitFilterBar: React.FC<CockpitFilterBarProps> = ({
  filters,
  onFilterChange,
  onReset,
  loading = false,
  ...
  const handleChange = (key: keyof CockpitFiltersState, value: string) => {
    onFilterChange({
      ...filters,
      [key]: value,
    });
    ...
    <div className="bg-card border border-border rounded-2xl p-4 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">Filtros Inteligentes</h3>
            <p className="text-[11px] text-muted-foreground">Filtre o cockpit por período, gerente, regional, canal ou rede</p>
          </div>
        </div>
