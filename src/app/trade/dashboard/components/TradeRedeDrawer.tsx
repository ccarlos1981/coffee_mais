...
export interface TradeRedeDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  context: {
    pdvNome: string;
...
export function TradeRedeDrawer({
  isOpen,
  onClose,
  context,
  onOpenFollowUp,
}: TradeRedeDrawerProps) {
  const [selectedNetwork, setSelectedNetwork] = useState<string | null>(
    context.codigoMatriz || context.redeNome || context.pdvId || null
  );
  ...
  useEffect(() => {
    setSelectedNetwork(context.codigoMatriz || context.redeNome || context.pdvId || null);
  ...
  const tradeContextSlug = (context.tipoCompliance || context.tipoOcorrencia || context.missaoNome || "TRADE")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-zA-Z0-9]/g, "_")
    .toUpperCase()
  ...
  const handleCreateFollowUp = () => {
    ...
    onOpenFollowUp({
      clienteNome: selectedNetwork || context.pdvNome || "PDV de Trade",
      origem: "COCKPIT_PRESCRITIVO",
      origem_ref: `TRADE_PDV_${cleanCod}_${pdvSlug}_${context.dataStr}_${tradeContextSlug}`,
      descricao: `Ocorrência/Compliance de Trade registrado no PDV ${context.pdvNome} (${context.redeNome || 'Sem Rede'}) pelo promotor ${context.promotorNome || 'Promotor'}: ${context.descricaoOcorrencia || context.tipoCompliance || context.tipoOcorrencia || 'Acompanhamento de conformidade operacional'}.`,
    });
    onClose();
  };
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Diagnóstico 360 do PDV e Trade"
      >
        {/* Header */}
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3 shadow-md">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs pt-1">
              ...
              {context.tipoCompliance && (
                <div className="col-span-2 flex items-center gap-2 text-slate-400">
                  <ShieldAlert className="w-4 h-4 text-rose-400 shrink-0" />
                  <span>Compliance: <strong className="text-rose-400">{context.tipoCompliance}</strong></span>
                </div>
              )}
              {context.tipoOcorrencia && (
                <div className="col-span-2 flex items-center gap-2 text-slate-400">
                  <AlertTriangle className="w-4 h-4 text-amber-400 shrink-0" />
                  <span>Ocorrência: <strong className="text-amber-400">{context.tipoOcorrencia}</strong></span>
                </div>
              )}
              {context.missaoNome && (
                <div className="col-span-2 flex items-center gap-2 text-slate-400">
                  <Layers className="w-4 h-4 text-purple-400 shrink-0" />
                  <span>Missão: <strong className="text-purple-400">{context.missaoNome}</strong></span>
                </div>
              )}
            </div>
          </div>
          ...
          {onOpenFollowUp && (
            <div className="pt-4 border-t border-slate-800">
              ...
              >
                ...
                Criar Ação de Follow-up (Trade & Campo)
              </button>
            </div>
          )}
        </div>
