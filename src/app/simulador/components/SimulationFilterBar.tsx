...
interface SimulationFilterBarProps {
  ...
  onFilterChange: (newFilters: SimulationFiltersState) => void;
  onReset: () => void;
  loading?: boolean;
}

export const SimulationFilterBar: React.FC<SimulationFilterBarProps> = ({
  filters,
  onFilterChange,
  onReset,
  loading = false,
  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        console.error("Erro ao carregar filtros do Simulador:", err);
      }
    }
    loadOptions();
  }, []);

  const handleChange = (key: keyof SimulationFiltersState, value: string) => {
    onFilterChange({
      ...filters,
      [key]: value,
    });
    ...
    <div className="bg-card border border-border rounded-2xl p-4 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">Filtros Oficiais da Simulação</h3>
            <p className="text-[11px] text-muted-foreground">Filtre a base analítica que servirá de ponto de partida</p>
          </div>
        </div>
