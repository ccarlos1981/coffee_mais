...
interface SimulationRedeGridProps {
  redes: SimulationDimensional[];
  loading?: boolean;
}

export const SimulationRedeGrid: React.FC<SimulationRedeGridProps> = ({
  redes,
  loading = false,
}) => {
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h3 className="text-sm font-bold text-foreground">Impacto Simulado por Rede / Matriz</h3>
          <p className="text-[11px] text-muted-foreground">Efeito da simulação nas maiores contas</p>
        </div>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-border">
        <table className="w-full text-left text-xs font-mono">
          <thead className="bg-muted/50 text-muted-foreground uppercase font-semibold text-[10px] tracking-wider border-b border-border">
            <tr>
              <th className="py-2.5 px-3 font-sans">Rede / Matriz</th>
          ...
          <tbody className="divide-y divide-border">
            {redes.map((r) => (
              <tr key={r.id} className="hover:bg-muted/30 transition-colors">
                <td className="py-2.5 px-3 font-sans font-bold text-foreground truncate max-w-[200px]">{r.nome}</td>
