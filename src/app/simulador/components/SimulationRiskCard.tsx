...
interface SimulationRiskCardProps {
  riscos: SimulationRiscoOportunidade[];
  onSelect: (item: SimulationRiscoOportunidade) => void;
  loading?: boolean;
}

export const SimulationRiskCard: React.FC<SimulationRiskCardProps> = ({
  riscos,
  onSelect,
  loading = false,
}) => {
  ...
  return (
    <div className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h3 className="text-sm font-bold text-foreground">Riscos da Decisão Simulada</h3>
          <p className="text-[11px] text-muted-foreground">Alertas críticos prévios à execução</p>
        </div>
      </div>
        ...
        {riscos.map((r) => (
          ...
          >
            <div className="flex justify-between text-xs">
              ...
              <span className="font-mono font-bold text-rose-500">-{formatCur(r.impactoR$)}</span>
            </div>
