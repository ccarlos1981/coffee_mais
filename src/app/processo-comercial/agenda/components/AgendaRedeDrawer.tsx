...
export interface AgendaRedeDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  event: {
    dateStr: string;
...
export function AgendaRedeDrawer({
  isOpen,
  onClose,
  event,
  onOpenFollowUp,
}: AgendaRedeDrawerProps) {
  const [selectedNetwork, setSelectedNetwork] = useState<string | null>(
    event.codigoMatriz || event.redeNome || null
  );
  ...
  // Sincronizar seleção quando evento mudar
  useEffect(() => {
    setSelectedNetwork(event.codigoMatriz || event.redeNome || null);
  }, [event.codigoMatriz, event.redeNome, event.dateStr, event.manager]);
  ...
  const cleanCod = (selectedNetwork || "VISITA")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-zA-Z0-9]/g, "_")
    .toUpperCase();

  const managerSlug = (event.manager || "GERENTE")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-zA-Z0-9]/g, "_")
    .toUpperCase();
  ...
  const handleCreateFollowUp = () => {
    ...
    onOpenFollowUp({
      clienteNome: selectedNetwork || event.description || "Cliente da Visita",
      origem: "COCKPIT_PRESCRITIVO",
      origem_ref: `AGENDA_VISITA_${cleanCod}_${event.dateStr}_${managerSlug}`,
      descricao: `Compromisso de visita comercial realizada em ${event.dateStr} (${event.description}) sob gestão de ${event.manager}.`,
    });
    onClose();
  };
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Diagnóstico 360 da Visita - ${event.dateStr}`}
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-100 leading-tight">
                Diagnóstico 360° da Visita
              </h2>
              <div className="text-xs text-slate-400 mt-0.5">
                Data: {event.dateStr} • Gerente: {event.manager}
              </div>
            </div>
          </div>
          <button
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3 shadow-md">
            ...
            <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3 text-xs space-y-2">
              <div className="text-slate-200 font-medium whitespace-pre-wrap">
                {event.description || "Sem descrição informada para esta data."}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs pt-1">
              <div className="flex items-center gap-2 text-slate-400">
                ...
                <span>Responsável: <strong className="text-slate-200">{event.manager}</strong></span>
              </div>
              <div className="flex items-center gap-2 text-slate-400">
                <Calendar className="w-4 h-4 text-amber-500 shrink-0" />
                <span>Data: <strong className="text-slate-200">{event.dateStr}</strong></span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-2">
              ...
              Farol Comercial & Financeiro Pré-Visita (Wave B.9)
            </h4>
            ...
            {!selectedNetwork ? (
              <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-4 space-y-2">
                ...
                <p className="text-xs text-slate-400 leading-relaxed">
                  A rota atual possui descrição textual livre sem identificador oficial de rede associado.
          ...
          {onOpenFollowUp && (
            <div className="pt-4 border-t border-slate-800">
              ...
              >
                ...
                Criar Ação de Follow-up (Visita Comercial)
              </button>
            </div>
          )}
        </div>
