...
import { SlidersHorizontal, RotateCcw, Calendar, User, MapPin, Tag, Building2 } from "lucide-react";

export interface InteligenciaFiltersState {
  startMonth: string;
  endMonth: string;
  manager: string;
  uf: string;
...
interface InteligenciaFilterBarProps {
  filters: InteligenciaFiltersState;
  onFilterChange: (newFilters: InteligenciaFiltersState) => void;
  onReset: () => void;
  loading?: boolean;
}

export const InteligenciaFilterBar: React.FC<InteligenciaFilterBarProps> = ({
  filters,
  onFilterChange,
  onReset,
  loading = false,
  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        console.error("Erro ao carregar opções de filtro Inteligência Comercial:", err);
      }
    }
    loadOptions();
  }, []);

  const handleChange = (key: keyof InteligenciaFiltersState, value: string) => {
    onFilterChange({
      ...filters,
      [key]: value,
    });
    ...
    <div className="bg-card border border-border rounded-2xl p-4 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-bold text-foreground">Filtros do Centro de Inteligência</h3>
            <p className="text-[11px] text-muted-foreground">Filtros consolidados de alta precisão analítica</p>
          </div>
        </div>
