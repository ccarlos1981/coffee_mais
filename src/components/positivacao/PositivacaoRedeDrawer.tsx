...
export interface PositivacaoRedeDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  context: {
    clienteNome: string;
    ...
    volumeReal?: number;
    mesesPositivados?: number;
    mesesSemCompra?: number;
    historicoPositivacao?: unknown;
    dataStr: string;
  };
  onOpenFollowUp?: (context: FollowUpInitialContext) => void;
}

export function PositivacaoRedeDrawer({
  isOpen,
  onClose,
  context,
  onOpenFollowUp,
}: PositivacaoRedeDrawerProps) {
  ...
  const cleanCod = (context.codigoMatriz || context.codParceiro || context.clienteId || "POS")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
  ...
  const handleCreateFollowUp = () => {
    ...
    onOpenFollowUp({
      clienteNome: context.clienteNome || selectedNetwork || "Cliente Positivação",
      origem: "COCKPIT_PRESCRITIVO",
      origem_ref: `POSITIVACAO_${cleanCod}_${clienteSlug}_${context.dataStr}`,
      descricao: `Ação de Positivação e Recuperação de Cobertura para ${context.clienteNome} (Rede: ${context.redeNome || 'Direto'} | Gerente: ${context.gerenteNome || 'Comercial'} | Faturamento: ${formatCurrency(context.faturamentoReal || 0)}). Alinhamento comercial para garantir regularidade de compra e mix de produtos.`,
    });
    onClose();
  };
  ...
  return (
    <>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Diagnóstico 360 de Positivação"
      >
        {/* Header */}
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-100 leading-tight">
                Diagnóstico 360° de Positivação
              </h2>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 space-y-3 shadow-md">
            ...
            {/* Grid de Positivação & Faturamento */}
            <div className="grid grid-cols-2 gap-3 text-xs pt-1">
              ...
              <div className="bg-slate-950/50 border border-slate-850 p-2.5 rounded-xl">
                ...
                <div className="text-sm font-bold text-emerald-400 mt-1">
                  {context.mesesPositivados !== undefined ? `${context.mesesPositivados} mês(es)` : "Ativo"}
                </div>
              </div>
              ...
              <div className="bg-slate-950/50 border border-slate-850 p-2.5 rounded-xl">
                ...
                <div className="text-sm font-bold text-rose-400 mt-1">
                  {context.mesesSemCompra !== undefined ? `${context.mesesSemCompra} mês(es)` : "0"}
                </div>
              </div>
            </div>
          </div>
          ...
          {onOpenFollowUp && (
            <div className="pt-4 border-t border-slate-800">
              ...
              >
                ...
                Criar Ação de Follow-up (Reativação de Positivação)
              </button>
            </div>
          )}
        </div>
